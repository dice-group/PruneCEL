 ...
 * 
 * <p>
 * This class is integrated with the collection framework (as a set of
 * {@link Index indices} and obeys the collection semantic for methods such as
 * {@link #size} (cardinality) or {@link #equals} (same set of indices).
 * </p>
 * 
...
public class FastBitSet extends FastCollection<Index> implements Set<Index>, Reusable {
    ...
    private long[] bits;
    /**
     * Holds the length in words (long) of this bit set. Any word at or above the
     * current length should be ignored (assumed to be zero).
     */
    private int _length;
    ...
    /**
     * Creates a bit set of specified initial capacity (in bits). All bits are
     * initially {@code false}. This constructor reserves enough space to represent
     * the integers from {@code 0} to {@code bitSize-1}.
     * 
    ...
    /**
     * Returns a new, preallocated or {@link #recycle recycled} set instance (on the
    ...
    /**
     * Recycles a set {@link #newInstance() instance} immediately (on the stack when
     ...
     * <code>set(index.intValue())</code>.
     * 
     * @param index the integer value to be appended to this set.
     * @return {@code true} if this set did not contains the specified index;
     ...
     * This means it builds the intersection of the two sets. The result is stored
     * into this bit set.
     *
    ...
    /**
     * Performs the logical AND operation on this bit set and the complement of the
     * given bit set. This means it selects every element in the first set, that
     * isn't in the second set. The result is stored into this bit set.
     *
     * @param that the second bit set
     */
    ...
    /**
     * Returns the number of bits set to {@code true} (or the size of this set).
     *
    ...
    /**
     * Removes the specified integer value from this set. That is the corresponding
     * bit is cleared.
     *
    ...
    /**
     * Returns {@code true}> if the specified integer is in this bit set;
     * {@code false } otherwise.
     *
     * @param bitIndex a non-negative integer.
    ...
    /**
     * Returns {@code true} if this bit set shares at least one common bit with the
     * specified bit set.
     *
    ...
    /**
     * Returns the logical number of bits actually used by this bit set. It returns
     * the index of the highest set bit plus one.
     * 
     * <p>
     * Note: This method does not return the number of set bits which is returned by
     * {@link #size}
     * </p>
     *
     ...
     * (inclusive). If there is none, {@code -1} is returned. The following code
     * will iterates through the bit set:[code] for (int i=nextSetBit(0); i >= 0; i
     * = nextSetBit(i)) { ... }[/code]
     *
     * @param fromIndex the start location.
     * @return the first {@code false} bit.
    ...
    /**
     * Performs the logical OR operation on this bit set and the one specified. In
     * other words, builds the union of the two sets. The result is stored into this
     * bit set.
     *
     * @param that the second bit set.
     */
    ...
    /**
     * Sets the bits between from (inclusive) and to (exclusive) to the specified
     * value.
     *
     ...
     * 
     * <P>
     * Note: Unlike {@code java.util.BitSet} this method does not returns an
     * approximation of the number of bits of space actually in use. This method is
     * compliant with java.util.Collection meaning for size().
     * </p>
     *
    ...
    /**
     * Performs the logical XOR operation on this bit set and the one specified. In
     * other words, builds the symmetric remainder of the two sets (the elements
     * that are in one set, but not in the other). The result is stored into this
     * bit set.
     *
     * @param that the second bit set.
     */
    ...
    /**
     * Determines the cardinality of the bit set that would have been created when
     * performing the {@link #and(FastBitSet)}.{@link #cardinality()} methods on the
     * first bitset, without changing the two given bitsets or allocating the memory
     * for copying them.
     * 
     * @param bitset1
     * @param bitset2
     * @return
     */
    public static int andCardinality(FastBitSet bitset1, FastBitSet bitset2) {
        final int n = MathLib.min(bitset1._length, bitset2._length);
        int sum = 0;
        for (int i = 0; i < n; ++i) {
            sum += MathLib.bitCount(bitset1.bits[i] & bitset2.bits[i]);
        }
        return sum;
    }
}
